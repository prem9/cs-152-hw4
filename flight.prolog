...
% Helpers
time_in_minutes(H:M, Minutes) :-
    Minutes is H * 60 + M.

before_time(TimeA, TimeB) :-
    time_in_minutes(TimeA, MinA),
    time_in_minutes(TimeB, MinB),
    MinA < MinB.

% Main predicate
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, [Departing, Arriving], Cost) :-
    flight(Departing, Arriving, DepartureTime, ArrivalTime, Cost),
    \+ member(Departing, Visited),
    \+ member(Arriving, Visited).

route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, [Departing|RestFlights], TotalCost) :-
    flight(Departing, Intermediate, DepartureTime, ArrivalIntermediate, Cost1),
    \+ member(Departing, Visited),
    \+ member(Intermediate, Visited),
    route(Intermediate, Arriving, [Departing|Visited], DepartureIntermediate, ArrivalTime, RestFlights, Cost2),
    before_time(ArrivalIntermediate, DepartureIntermediate),
